...
const OperatorDashboardScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  // Easter egg / Demo feature: Click the status to toggle system state visual
      ...
      <div className="px-4 space-y-6">
        ...
        <section className="glass-card rounded-2xl border border-white/10 overflow-hidden relative group">
          ...
          <div className="relative h-64 bg-slate-900 w-full overflow-hidden">
            ...
            {/* Structures */}
            <div className="absolute top-8 left-8 w-24 h-40 border-2 border-slate-700 bg-slate-800/50 rounded-lg"></div> {/* Check-in */}
            <div className="absolute top-8 right-8 w-24 h-40 border-2 border-slate-700 bg-slate-800/50 rounded-lg"></div> {/* Security */}
            
            {/* Heatmap Overlays */}
            {/* Hot Zone - Security */}
            <div className={`absolute top-12 right-12 w-20 h-20 rounded-full blur-2xl animate-pulse transition-colors duration-1000 ${systemStatus === 'nominal' ? 'bg-emerald-500/20' : 'bg-red-500/40'}`}></div>
            {/* Moderate Zone - Check-in */}
            <div className="absolute top-16 left-12 w-16 h-16 bg-yellow-500/20 rounded-full blur-xl"></div>
          ...
          <div className="absolute inset-0 border-2 border-blue-500/0 group-hover:border-blue-500/20 pointer-events-none transition-all rounded-2xl"></div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            {/* Dynamic Alert based on System Status */}
